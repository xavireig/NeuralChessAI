...
data_path = 'data\\data2014-2020.h5'
validation_data_path = 'data\\data2013.h5'
variant = 'to'
network = '128-256-1024-512'
batch_size = 6561

model_path = 'models\\'+variant+'-withTurn-b'+str(batch_size)+'-'+network+'-model.h5'
# model_path = 'models\\to-withTurn-b2048-128-256-1024-1024-model.h5'
model_path_json = 'models\\'+variant+'-withTurn-b'+str(batch_size)+'-'+network+'-model.json'

# log_dir = "logs/" + variant + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
log_dir = 'logs/'+variant+'-b'+str(batch_size)+'-'+network

# training data
with h5py.File(data_path, "r") as h5f:
    num_samples = len(h5f['moved_'+variant])
    
steps_per_epoch = np.ceil(num_samples/batch_size)
training_data_batch_generator = batch_generator(data_path, batch_size, steps_per_epoch, variant)

# validation data
with h5py.File(validation_data_path, "r") as h5f:
    validation_num_samples = len(h5f['moved_'+variant])
    
validation_steps_per_epoch = np.ceil(validation_num_samples/batch_size)
validation_data_batch_generator = batch_generator(validation_data_path, batch_size, validation_steps_per_epoch, variant)
